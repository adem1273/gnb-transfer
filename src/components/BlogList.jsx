                ...
                <div key={blog._id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
                    <div className="p-4">
                        <Link to={`/blog/${blog._id}`}>
                            <h2 className="text-xl font-bold text-gray-800 mb-2 hover:text-blue-600 transition-colors">{getTranslatedTitle(blog)}</h2>
                        </Link>
                        <p className="text-gray-600 mb-4">
                            {getTranslatedContent(blog).substring(0, 150)}...
                        </p>
                        <Link 
                            to={`/blog/${blog._id}`} 
